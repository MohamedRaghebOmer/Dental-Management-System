...
public sealed class VisitViewRepository : IVisitViewRepository
{
    ...
    //        whereClauses.Add("""
    //            ABS(
    //                (COALESCE(ta.TotalAmount, 0) - (COALESCE(pa.SumOfPaidAmounts, 0) + COALESCE(v.DiscountAmount, 0)))
    //                - @RemainedAmount
    //            ) < 0.005
    //            """);
    ...
    public async Task<List<VisitView>> GetAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        IQueryable<Domain.Entities.Visit> query = _dbContext.Visits
            .AsNoTracking();
        ...
        if (filterDto is not null)
        {
            ...
            {
                var id = Id.FromDatabase(visitId);
            ...
            {
                var id = Id.FromDatabase(appointmentId);

                query = query.Where(v => v.AppointmentId == id);
            }

            if (filterDto.PatientId is { } patientId)
            {
                var id = Id.FromDatabase(patientId);

                query = query.Where(v => v.PatientId == id);
            }

            if (!string.IsNullOrWhiteSpace(filterDto.PatientName))
            {
                var patientName = filterDto.PatientName.Trim();

                query = query.Where(v =>
                    v.Patient.Name.Contains(patientName));
            }
            ...
            if (!string.IsNullOrWhiteSpace(filterDto.VisitTreatmentsNames))
            {
                ...
                foreach (var treatmentName in treatmentNames)
                {
                    var name = treatmentName.Trim();

                    query = query.Where(v =>
                        v.VisitTreatments.Any(vt =>
                            vt.Treatment.Name.Contains(name)));
                }
            }

            if (filterDto.VisitDateTime is { } visitDateTime)
            {
                var date = visitDateTime.Date;
                var nextDate = date.AddDays(1);

                query = query.Where(v =>
                    v.VisitDateTime >= date &&
                    v.VisitDateTime < nextDate);
            }
            else if (filterDto.GetViewsAfterDateTime is { } afterDateTime)
            {
                query = query.Where(v =>
                    v.VisitDateTime >= afterDateTime);
            }

            if (filterDto.DiscountAmount is { } discountAmount)
            {
                var money = Money.FromDatabase(discountAmount);

                query = query.Where(v =>
                    v.DiscountAmount == money);
            }
        }
        ...
        var visitEntities = await query
            ...
            .Include(v => v.VisitPayments)
            .AsSplitQuery()
            .ToListAsync(cancellationToken);

        IEnumerable<VisitView> visits = visitEntities
            .OrderByDescending(v => v.VisitDateTime)
            .ThenBy(v => v.Id.Value)
            .Select(MapToVisitView);
        ...
        if (filterDto is not null)
        {
            if (filterDto.TotalAmount is { } totalAmount)
            {
                visits = visits.Where(v =>
                    Math.Abs((v.TotalAmount ?? 0m) - totalAmount) < 0.005m);
            }

            if (filterDto.SumOfPaidAmounts is { } sumOfPaidAmounts)
            {
                visits = visits.Where(v =>
                    Math.Abs((v.SumOfPaidAmounts ?? 0m) - sumOfPaidAmounts) < 0.005m);
            }

            if (filterDto.RemainedAmount is { } remainedAmount)
            {
                visits = visits.Where(v =>
                    Math.Abs((v.RemainedAmount ?? 0m) - remainedAmount) < 0.005m);
            }
        }

        return visits.ToList();
    }
    private static VisitView MapToVisitView(Domain.Entities.Visit v)
    {
        var totalAmount = v.VisitTreatments.Sum(vt => vt.TreatmentPrice.Value * vt.Count);
        var paidAmount = v.VisitPayments.Sum(vp => vp.PaidAmount.Value);

        return new VisitView
        {
            VisitId = v.Id.Value,
            AppointmentId = v.AppointmentId?.Value,
            PatientId = v.PatientId.Value,
            PatientName = v.Patient.Name,
            VisitTreatmentsNames = string.Join("، ", v.VisitTreatments.Select(vt => vt.Treatment.Name)),
            VisitDateTime = v.VisitDateTime,
            TotalAmount = totalAmount,
            SumOfPaidAmounts = paidAmount,
            DiscountAmount = v.DiscountAmount.Value,
            RemainedAmount = totalAmount - paidAmount - v.DiscountAmount.Value
        };
    }
